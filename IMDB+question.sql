...
SELECT count(*) as Directormapping_row_count FROM director_mapping ;
-- Number of rows = 3867
...
SELECT count(*) as rolmapping_row_count FROM role_mapping ;
...
SELECT sum(id IS NULL) AS Id_null_count,
        ...
        sum(worlwide_gross_income IS NULL) AS Worldwide_gross_income_null_count,
...
FROM movie
where year = 2019 AND (country regexp "India.*" OR country regexp "USA.*");
...
select DISTINCT genre as Unique_List_Genre
from genre;
...
+---------------+-------------------+---------------------+*/
-- Type your code below:
-- using CTE to  Finds the rank of each genre based on the number of movies in each genre
...
-- Thriller Genre has rank=3 and movie count of 1484
...
-- top 10 movies is displayed using WHERE caluse with CTE
...
-- 24 Drama movies were released during March 2017 in the USA with more than 1,000 votes
...
-- Total 8 movies  begin with "The" in their title.
...
-- Top three production houses based on the number of votes for their movies are Marvel Studios, Twentieth Century Fox and Warner Bros.
...
WITH highest_grossing_movies AS (
        ...
        FROM
                ...
                ON m.id= g.movie_id 
                INNER JOIN
                top_three_genre AS thg
                ON g.genre = thg.genre                         
) SELECT 
        *
  FROM 
...
WITH multilingual_movies_count_ranking AS (
        ...
        SELECT 
                ...
                COUNT(m.id) AS movie_count,
                ROW_NUMBER() OVER(ORDER BY COUNT(m.id) DESC) AS production_company_rank
        FROM
                movie AS m
                INNER JOIN ratings AS r
                ON m.id = r.movie_id
...
SELECT *
FROM multilingual_movies_count_ranking
WHERE
